...
def home(request):
    ...
    if category:
        products = Product.objects.filter(category=category, active=True).select_related('store')
    else:
...
def initiate_payment(request, order_id):
    ...
    if request.method == 'POST':
        ...
        if not phone:
            messages.error(request, 'Enter a valid phone number (e.g. 07XXXXXXXX or 255XXXXXXXXX).')
            return redirect('initiate_payment', order_id=order.id)
        ...
        return render(request, 'payment/processing.html', {'order': order, 'payment': payment})
...
def payment_status(request, payment_id):
    payment = get_object_or_404(Payment, id=payment_id, order__fundi=request.user.profile)
...
def register(request):
    if request.method == 'POST':
        ...
        if not phone:
            return render(request, 'register.html', {'error': 'Use a valid phone number (07XXXXXXXX or 255XXXXXXXXX).'})
        if len(password) < 6:
            return render(request, 'register.html', {'error': 'Password must be at least 6 characters long.'})
        if role == 'hardware' and not store_name:
            return render(request, 'register.html', {'error': 'Store name is required for hardware accounts.'})

        if User.objects.filter(username=phone).exists():
           return render(request, 'register.html', {'error': 'Phone number already registered. Please login.'})

        try:
            with transaction.atomic():
                user = User.objects.create_user(username=phone, password=password)
                profile = Profile.objects.create(user=user, phone=phone, full_name=full_name, role=role, area=area)

                if role == 'hardware':
                    HardwareStore.objects.create(owner=profile, name=store_name, area=area)
...
def dashboard(request):
    ...
    if profile.role == 'fundi':
        ...
        return render(request, 'dashboard/fundi.html', {'profile': profile, 'orders': orders})
        ...
        return render(request, 'dashboard/hardware.html', {'profile': profile, 'store': store, 'products': products, 'orders': orders})
    else:
...
def add_product(request):
    ...
    if profile.role != 'hardware':
        logger.warning(f"Non-hardware user {request.user.username} tried to add product")
        return redirect('dashboard')
...
def manage_order(request, order_id, action):
    ...
    if profile.role != 'hardware':
        logger.warning(f"Non-hardware user {request.user.username} tried to manage order")
        return redirect('dashboard')
    ...
    try:
        ...
        logger.info(f"Order {order_id} status changed from {old_status} to {new_status} by {request.user.username}")
...
def manage_page(request):
    try:
        ...
        if request.user.is_superuser:
            profile = Profile.objects.create(user=request.user, phone=request.user.username, full_name='Super Admin', role='admin', area='All')
        else:
            return redirect('home')
    ...
    if request.method == 'POST':
        ...
        if action == 'add_hardware' and profile.role == 'admin':
            ...
            if not all([phone, full_name, store_name, area, password]):
                messages.error(request, 'All hardware fields are required with a valid phone number.')
                return redirect('manage_page')

            user = User.objects.filter(username=phone).first()
            if not user:
                user = User.objects.create_user(username=phone, password=password)
                new_profile = Profile.objects.create(user=user, phone=phone, full_name=full_name, role='hardware', area=area)
            else:
