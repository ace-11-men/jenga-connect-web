...
from .models import Profile, HardwareStore, Product, CommissionSetting, Order, OrderItem
...
class HardwareStoreAdmin(admin.ModelAdmin):
    list_display = ('name', 'owner', 'area', 'delivery_capacity_units_per_day', 'active')
    list_filter = ('active', 'area')
    search_fields = ('name', 'owner__full_name')

@admin.register(Product)
class ProductAdmin(admin.ModelAdmin):
    list_display = ('name', 'store', 'category', 'unit', 'hardware_price_per_unit', 'stock_units', 'active')
    list_filter = ('category', 'active', 'unit')
    search_fields = ('name', 'store__name')

@admin.register(CommissionSetting)
class CommissionSettingAdmin(admin.ModelAdmin):
    list_display = ('category', 'unit', 'commission_per_unit', 'effective_from', 'active')
